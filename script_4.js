...
console.log("Il y a " + howmuchNumber(journas) + " handles contenant un numéro.");


// Combien d'handles contiennent les 4 lettres du prénom "Aude" à la suite (sans prendre en compte les majuscules) ?
function gotAude(array) {
	let i = 0;
	array.forEach(handle => {
		handle = handle.toLowerCase();
		if (handle.includes("aude")) {
			i ++;
		}
	})
	return i;
...
console.log("Il y a " + gotAude(journas) + " handles contenant 'aude'.");


// Combien commencent par une majuscule (première lettre juste après le @) ?
function startWithUpcase(array) {
	let i = 0;
	array.forEach(handle => {
		if(handle[1].toUpperCase() === handle[1]) {
			i ++;
		}
	})
	return i;
...
console.log("Il y a " + startWithUpcase(journas) + " handles commençant par une majuscule.");


// Combien contiennent une majuscule ?
function includesUpcase(array) {
	let i = 0;
	array.forEach(handle => {
...
console.log("Il y a " + includesUpcase(journas) + " handles contenant une majuscule.");
